<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="jhs_web.*, java.util.*, java.text.*"%>
...
<script type="text/javascript">
	function delCheck(board_id) {
		...
		if(result == true){
			document.location.href="board_control.jsp?action=delete&board_id="+parseInt(board_id);
		}
		else
			return;
	}
	...
	function updateCheck(board_id) {
		document.location.href="board_control.jsp?action=edit&board_id="+parseInt(board_id);
	}
</script>
...
<br><br>
<%
	if(current_user.getUs_identification().equals("admin")) {
	...
	else if(board.getBoard_user_identification().equals(current_user.getUs_identification())) {
		out.println("<input type='button' value='삭제' onClick='delCheck("+board.getBoard_id()+")'/>");
		out.println("<input type='button' value='수정' onClick='updateCheck("+board.getBoard_id()+")'/>");
	}
%>
...
<input type="hidden" name="comment_from" value="<%=board.getBoard_id()%>"/>
<input type="hidden" name="comment_author" value="<%=current_user.getUs_identification() %>"/>
<%
	Date date = new Date();
	SimpleDateFormat DateFor = new SimpleDateFormat("yy.MM.dd(hh시 mm분)");
	String stringDate= DateFor.format(date);
	
	out.println("<input type='hidden' name='comment_date' value='"+stringDate+"'/>");
%>
댓글달기  <input type="text" name="comment_contents" style="width:60%;height:30px;font-size:45;"/>
